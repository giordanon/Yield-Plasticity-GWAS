...
planting <- read.csv("../../data/environment/planting_dates2000_2017.csv")[,1:2]


locations <- unique(dataVPT[,2:4]) %>% 
  left_join(read.csv("../../data/environment/trials_coordinates.csv"), by = join_by(L)) %>% 
  dplyr::select(-GY) %>% 
  unique() %>% 
  mutate(Y = as.numeric(str_extract(E,"\\d+\\.?\\d*")), 
         sY = as.Date(paste0(Y-1, "-01-01"), format = "%Y-%m-%d")
         ) %>% 
  left_join(planting) %>% 
  mutate(PD = sY + SD)
...
- Get weather data for CP of yield determination. 
- Get weather data for conditions during fall: accumulated rain and GDD during fall tillering stages.
- Get weather data for winter: number of days with temps below 0 C, rainfall. 
...
```{r}
# weather_daymet_2.RData this file considers the trial year as the planting year
...
locations[,] %>%
  ...
  mutate(.,
         ...
         daymet = pmap(list(..1 = LAT, ..2 = LON, ..3 = Y+1, ..4 = Y+2, ..5 = E),
...
saveRDS(env, file = "../../data/environment/weather_daymet_2.RData")
